...
pub fn fs_error_msg(code: u64) -> &'static [u8] {
    match code {
        ...
        syscall_abi::FS_ERR_NO_SUCH_VERB => b"that server does not implement this request, or not on this kind of connection (its log names the verb)",
        syscall_abi::FS_ERR_BUSY => b"that server is out of room for this right now (try again later)",
