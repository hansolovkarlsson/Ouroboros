...
fn serve(packed_mac: u64) -> ! {
    ...
    // survives from a C program's open() to its close(). The same parked engine
    // as `remotes` below since step 2 of the async plan, kept Established
    // between verbs; on this frame like `conns`/`dials` - no mutable statics.
    let mut sessions: [Option<SessionConn>; MAX_CLIENT_SESSIONS] = core::array::from_fn(|_| None);
    ...
    loop {
        ...
        let unacked = conns
            ...
            || sessions.iter().any(|c| matches!(c, Some(r) if r.parked.is_some() || r.state == DialState::Closing || r.state == DialState::Closed));
        ...
        if packed_mac != syscall_abi::NET_ERROR {
            while let Some(n) = recv(&mut frame) {
                on_frame(&mac, &frame[..n], &mut conns, &mut dials, &mut remotes, &mut sessions, &auth);
            }
            // Answer every parked remote mount and session verb whose reply is
            // complete (or whose peer is gone), then drive the client
            // connections (SYN/data/FIN retransmits, idle GC). Service BEFORE
            // pump: the pump reaps a Closed slot whose buffer is empty, and a
            // reply must be delivered out of that buffer first. A path slot
            // that dies unanswered says NO_FS (an unreachable peer); a session
            // that dies under its fids says FS_ERROR (fail-dead, the word
            // `client_exchange` used for the same event before step 2).
            service_remotes(&mut remotes, &auth, syscall_abi::NO_FS, Some(&mut pending));
            service_remotes(&mut sessions, &auth, syscall_abi::FS_ERROR, None);
            pump_dials(&mac, &mut remotes, DIAL_IDLE_TICKS);
            // A held session idles for the far side's reap plus a margin (the
            // backstop for a client that crashed without clunking, Decision 4)
            // where a parked path verb or a dial idles for seconds.
            pump_dials(&mac, &mut sessions, CLIENT_SESSION_IDLE_TICKS);
            pump_dials(&mac, &mut dials, DIAL_IDLE_TICKS);
...
fn drain_client_messages(packed_mac: u64, buf: &mut [u8], conns: &mut [Option<TcpConn>; MAX_CONNS], dials: &mut [Option<DialConn>; MAX_DIAL], remotes: &mut [Option<RemoteConn>; MAX_REMOTE], sessions: &mut [Option<SessionConn>; MAX_CLIENT_SESSIONS], auth: &Auth, pending: &mut PendingRun) {
    loop {
        ...
        if let Some(ci) = child_conn {
            ...
            //
            // Until step 3 of the async plan this arm also had to refuse a
            // CHAINED cpu (a node both running a run and hosting a child),
            // because the child's mount would have parked on a slot the
...
fn handle_client(packed_mac: u64, sender: u64, buf: &[u8], len: usize, dials: &mut [Option<DialConn>; MAX_DIAL], remotes: &mut [Option<RemoteConn>; MAX_REMOTE], sessions: &mut [Option<SessionConn>; MAX_CLIENT_SESSIONS], auth: &Auth, pending: &mut PendingRun) {
    ...
    match read_u64(buf, 0) {
        ...
        syscall_abi::NETOP_RUN => {
            ...
            // its output back. PARKED since step 3 of the async plan (`None`
            // here: `service_remotes` answers the caller when the peer FINs),
            // so the serve loop keeps serving the spawned command's
...
/// DELIBERATELY NOT `#[inline(never)]`, unlike `park_rmount` and
/// `park_session`, and the difference was measured rather than reasoned. Those
/// two have a sibling branch with its own big buffers, so keeping each in its
/// own frame stops `handle_rmount` carrying both at once. This one's only
/// caller is `handle_client`, whose frame ALREADY holds the ping/resolve/fetch
/// buffers, and those lifetimes are disjoint from this one's: inlined, the
/// compiler overlaps them in the same slots. Adding the attribute here pushed
/// the deepest chain from 1936 to 3072 bytes past netd's guard page at 48 KB.
...
fn park_run(packed_mac: u64, buf: &[u8], len: usize, out: &mut [u8], remotes: &mut [Option<RemoteConn>; MAX_REMOTE], auth: &Auth, pending: &mut PendingRun) -> Option<usize> {
    ...
    // PARK it (step 3 of docs/roadmap/roadmap-async-rmount.md). This used to
    // call `tcp_run`, which drove the run's connection while re-entrantly
    // pumping the export and the mailbox, because a cpu child's Phase 4b
    // `/host` reads come back to *our* export as frames that must be served
    // DURING the run - a plain blocking read would have deadlocked. Parking
    // makes that machinery unnecessary rather than cheaper: the serve loop is
    // never blocked, so it serves the export, acks the health ping and drives
    // every other client while the run is in flight, and the re-entrant drain
    // (with its by-sender refusal, #147) is gone with it.
    //
    // The output is collected by `service_remotes`, which drains the slot's
...
fn park_remote(dst_mac: &[u8; 6], ip: [u8; 4], port: u16, verb: u64, caller: u64, who: &[u8; ninep_abi::NP_NAME_LEN], np: &[u8], remotes: &mut [Option<RemoteConn>; MAX_REMOTE]) -> Result<(), &'static [u8]> {
    ...
    // The run's reply is a STREAM ended by the peer's FIN, not a framed NP
    // reply, and it carries no signature (cpu output is reply-unauthenticated,
    // stated in the auth design). `verb == NP_RUN` is what tells
...
/// a node talks to a handful of peers. Each slot is a whole `SessionConn`
/// (its send and receive buffers plus the held first verb, ~2.7 KB) resident
/// on `serve`'s frame, so this trades directly against netd's stack headroom
/// like `MAX_DIAL` and `MAX_REMOTE`. A new session with the table full is
/// refused `FS_ERR_BUSY`, the same answer the far side's session budget gives,
/// never by evicting a live one.
const MAX_CLIENT_SESSIONS: usize = 3;

/// Ticks (`now()`) of inactivity before a held client session is reaped as a
/// BACKSTOP, the `idle_ticks` `pump_dials` runs the session table with.
/// Deliberately ABOVE `CONN_IDLE_TICKS` (the far side's 30 s reap): the normal
/// path closes a session on its last clunk, and this only cleans up after a
/// client that crashed without clunking - by which time the far side has
/// certainly reaped its half, so this never drops a live-but-slow session under
/// its holder (Decision 3's rule, applied to the client side). It has to be
/// per table: the engine's own `DIAL_IDLE_TICKS` (12 s) would reap a held
/// session between one C program's `open()` and its next `read()`.
...
/// nonce WE sent. The one place a client trusts a byte off the wire: every
/// parked reply, path verb or session verb, is verified through it from
/// `service_remotes`, so the check is spelled once (Decision 3's client-side
/// asymmetry - the key is the one for the address we dialled, not merely one
/// we authorize).
...
/// Park a FID verb on a held session slot (Decision 4 of
/// docs/roadmap/roadmap-fid-verbs.md, made asynchronous by step 2 of
...
/// nonce on a request that may never be sent. `Err` is the status to fail the
/// caller with now: `FS_ERR_AUTH` (no key for the peer, or nothing to sign
/// with), or `FS_ERR_BUSY` for a second verb on a session whose first is still
/// in flight, and for the slot budget. The overlap refusal is a STATED LIMIT
/// the blocking path made unspellable: two fid verbs from one uid to one
/// endpoint could never overlap while the first held the event loop. libc's
/// `read()` loop does not retry on `FS_ERR_BUSY`, so two C programs sharing
/// one remote mount under one user can see a read fail busy; that is the
/// limit, not a bug to chase, until a queue is worth its stack.
...
fn park_session(dst_mac: &[u8; 6], ip: [u8; 4], port: u16, uid: u32, verb: u64, caller: u64, who: &[u8; ninep_abi::NP_NAME_LEN], expect_key: Option<[u8; clusterkeys::KEY_LEN]>, auth: &Auth, msg: &[u8], sessions: &mut [Option<SessionConn>; MAX_CLIENT_SESSIONS]) -> Result<(), u64> {
    ...
    // one is spent - its fids died with the connection - and is not found, so
    // the verb opens a fresh session beside it until the pump reaps it.
    let live = sessions.iter().position(|s| matches!(s, Some(s) if s.peer_ip == ip && s.peer_port == port && s.uid == uid && matches!(s.state, DialState::Connecting | DialState::Established)));
    if let Some(i) = live {
        ...
        // client-session PR).
        s.peer_mac = *dst_mac;
        // One verb in flight per session. `slen != 0` with nothing parked would
        // mean the previous request was never fully acked before its reply
        // completed, which TCP does not do; refused rather than overwritten.
        if s.parked.is_some() || s.slen != 0 {
            return Err(syscall_abi::FS_ERR_BUSY);
        }
        // The caller's uid now resolves to another name than the one this
        // session was opened as (its /etc/passwd line changed meanwhile).
        // Refused, not run as the opener: on a keyed session the frame carries
        // no name, so the far side would serve it as whoever signed the
        // NP_SESSION (review of step 7). The session and its fids are left to
        // their holder; the refused caller can open a fresh one under the new
        // name once they are closed.
        if s.keys[0].opener != *who {
            return Err(syscall_abi::FS_ERR_AUTH);
        }
    ...
    // Built IN the slot, as `park_rmount` builds its `RemoteConn`: a
...
/// now: a refusal decided here, before any network work. `None` means the
/// request was PARKED and its reply will go out from `service_remotes` when
...
fn handle_rmount(packed_mac: u64, buf: &[u8], len: usize, sessions: &mut [Option<SessionConn>; MAX_CLIENT_SESSIONS], remotes: &mut [Option<RemoteConn>; MAX_REMOTE], out: &mut [u8], auth: &Auth) -> Option<usize> {
    ...
    // Every other verb carries its own path and needs no handle, so it gets a
    // connection of its own, opened and closed per call. Either way the request
...
///
/// Not inlined, and neither is `park_session`: with one caller each the
/// compiler folds a branch into `handle_rmount`, whose frame then carries that
/// branch's locals on EVERY remote mount. That is what overflowed the guard
/// page on the async rig's first parked request (measured 2026-09-21: the
/// then-synchronous session path, ~5 KB of buffers, inlined into
/// `handle_rmount`'s prologue once its second caller was deleted). The
/// attribute names the property the old "each branch holds its own buffers"
/// comment relied on without saying: two callers. The branches are small now
...
/// Answer the parked requests of one table: a slot whose framed reply is
/// complete has it verified (`verify_sealed`, the one place a client trusts a
/// byte off the wire) and delivered to its caller's IDENTITY; a slot whose peer
/// is gone (reset, retransmits exhausted) or whose deadline has passed answers
/// `dead` - `NO_FS` for the path table (the one-shot path's word for an
/// unreachable peer), `FS_ERROR` for the session table (fail-dead: the session
/// closes, its fids go with it, and the client re-opens on its next call,
/// exactly what `client_exchange` did after one second of silence before step
/// 2; a session still OPENING answers `NO_FS`, as its connect failure did).
/// Either way the parked record is cleared, so a caller is answered exactly
/// once. A caller that died while parked is refused by the kernel (its slot's
/// occupant changed) and the reply is dropped, which is the right thing to do
/// with it.
///
/// What happens to the connection afterwards is the fid refcount's call, and
/// it is the same rule for both tables: `NP_OPEN` success adds a fid,
/// `NP_CLUNK` success removes one, and a slot holding none closes (left for
/// `pump_dials` to FIN and reap). A path slot never carries a fid verb, so it
/// closes after its one reply; a session stays `Established` for the next verb
/// of its fid's life. A session closing at zero is also the freshly-opened-
/// but-the-open-failed close: a session opened for an `NP_OPEN` that returned
/// an error never reached one fid and would otherwise hold a slot and a
/// far-side connection for `CLIENT_SESSION_IDLE_TICKS` (review of the
/// client-session PR).
///
/// A session slot's reply may be the far side's answer to the `NP_SESSION`
/// that opens it (`Parked::opening`). Status 0 there starts PHASE TWO: the
/// verb held raw in `Wire::held` is signed now, on the session the far side
/// just accepted, and parked in its turn with a fresh nonce and a fresh
/// deadline; any other answer (an old export's `FS_ERR_NO_SUCH_VERB`, a full
/// one's `FS_ERR_BUSY`) is the caller's reply, and no session is kept. Signing
...
fn service_remotes<const H: usize, const K: usize>(remotes: &mut [Option<Wire<REMOTE_SBUF, REMOTE_RBUF, H, K>>], auth: &Auth, dead: u64, mut pending: Option<&mut PendingRun>) {
    ...
    // frame, because this is inlined into it and so sits beneath every chain
    // netd has; measured when step 3's run arm first declared its own.
    let mut r = [0u8; syscall_abi::MSG_MAX_LEN as usize];
    for slot in remotes.iter_mut() {
        ...
        if c.parked.as_ref().unwrap().raw != 0 && c.state != DialState::Closed {
            if !sign_parked(c, auth) {
                // A request too large to frame, or on a keyed session one
                // `frame_keyed_in_place` refused (too large, or `seq` would
                // overflow); `can_send` was asked at the park. Answered as the
                // park would have answered it: FS_ERR_AUTH.
        ...
        // not a framed NP reply, and it is reply-unauthenticated, so none of
        // the framing, length or signature logic below applies to it. Its
        // output is drained into `pending` as it arrives, which is what keeps
        // the slot's receive buffer small: `RUN_OUT_MAX` of output does not
        // have to fit `REMOTE_RBUF`.
        if c.parked.as_ref().unwrap().verb == ninep_abi::NP_RUN {
            let Some(p) = pending.as_deref_mut() else { continue };
            if c.rlen > 0 {
                let room = RUN_OUT_MAX.saturating_sub(p.len);
                let take = c.rlen.min(room);
                p.buf[p.len..p.len + take].copy_from_slice(&c.rbuf[..take]);
                p.len += take;
                // Consumed: the buffer is free for the next segment, and the
                // peer's window re-opens. Output past RUN_OUT_MAX is dropped,
                // the same bound `tcp_run` applied with `got.min(RUN_OUT_MAX)`.
                c.rlen = 0;
                // Progress: a run is bounded by SILENCE, not by total time.
                c.parked.as_mut().unwrap().since = t.max(1);
            }
            let finished = c.peer_fin || c.state == DialState::Closed;
            let since = c.parked.as_ref().unwrap().since;
            let silent = since != 0 && t.wrapping_sub(since) > RUN_DEADLINE_TICKS;
            if !(finished || silent) {
                continue;
            }
            let parked = c.parked.take().unwrap();
        ...
        let gone = c.state == DialState::Closed;
        let since = c.parked.as_ref().unwrap().since;
        let late = since != 0 && t.wrapping_sub(since) > REMOTE_DEADLINE_TICKS;
        if !(complete || too_big || gone || late) {
            continue;
        }
        let parked = c.parked.take().unwrap();
        ...
        let n = if complete && !too_big {
            let checked = match c.keys.first().map(|a| &a.state) {
                Some(KeyState::Keyed { k_s2c, seq, .. }) => verify_keyed_reply(&c.rbuf, c.rlen, k_s2c, *seq),
                _ => verify_sealed(&c.rbuf, c.rlen, &parked.nonce, &parked.expect_key),
            };
            match checked {
                Some((start, len)) => {
                    verified = true;
        ...
        c.rlen = 0;
        let np_status = if n >= 8 { read_u64(&r, 0) } else { u64::MAX };
        if parked.opening && verified && np_status == 0 && c.slen == 0 {
            ...
            // still-open connection is closed politely. The ephemeral secret
            // goes NOW, not at the reap: this is a refusal path, exactly where
            // a resident secret would go unwatched (review of step 7).
            c.held_len = 0;
            if let Some(ka) = c.keys.first_mut() {
                ka.forget();
            }
            ...
            if verified && c.fids > 0 {
                // A live session with fids: kept for the next verb.
                c.last_activity = t;
                continue;
            }
        }
        // Done with the connection: a live one is closed politely. A slot
        // still connecting has nothing to FIN; it is Closed outright.
        match c.state {
            DialState::Established => c.state = DialState::Closing,
            DialState::Closing | DialState::Closed => {}
            _ => c.state = DialState::Closed,
...
struct TcpConn {
    ...
    /// it, and either frees the slot, and with it every fid in `fids`.
    session: bool,
    /// The fids this connection holds (step 5): client-facing fid
    ...
    /// Whether a frame has already been dispatched on this connection. A key is
    /// accepted only on the FIRST (ninep-abi, "Keyed sessions"): an offer on a
    /// connection that has carried anything, signed or keyed, is refused and
    /// the connection closed, since keying mid-stream would leave the two ends
    /// disagreeing about the format of the next frame. A field rather than
    /// `!session`, which happens to coincide today only because a one-shot
    /// connection never dispatches a second frame.
    carried: bool,
...
/// How many fids one SESSION may hold at once (docs/roadmap/roadmap-fid-verbs.md,
/// step 5). Small and fixed: the table rides in `TcpConn`, which lives on
...
/// The smallest fid a session hands out. ninep-abi's `NP_OPEN` promises "a
/// small integer >= 3" (0-2 are a C program's stdio), and fsd numbers its own
/// from 3 for the same reason. Two private constants rather than one shared,
/// on purpose: a client-facing fid is netd's number and never fsd's (Decision
/// 2), and the two spaces starting at the same value is a coincidence a shared
/// name would read as an identity.
const FID_BASE: u64 = 3;

/// One fid a session holds: the fsd fid behind it, and WHO opened it.
#[derive(Clone, Copy)]
struct SessionFid {
    /// fsd's fid, or 0 for a free slot (fsd's are >= 3, so 0 is never one).
    fsd_fid: u64,
...
/// frees a slot: there are six of those (the peer's FIN, its RST, the idle
/// reap, the RTO give-up, a SYN rebuilding the slot, and the serve loop's own
/// copies of two of them), and the next one added would be the one that
/// forgot. This way the wrong thing is unspellable, not merely un-grepped
/// (docs/postmortems/unspellable-postmortem.md). Each clunk is one `MSG_CALL`
/// to fsd as the user who opened the fid, from the same event loop that made
/// the open; a connection that never opened anything costs nothing here. A fid
/// fsd no longer knows (fsd restarted) answers an error that is ignored: the
/// handle is gone either way.
///
/// The fids are the ONLY per-connection resource released here, and that is
/// the limit of the claim above. A running `cpu_child` is not: a `WAIT` on a
/// live child would block the event loop, and a `KILL` or a deferred reap is
/// its own change. So a peer that RSTs or FINs mid-run leaves the child to
/// finish into a connection that no longer exists, and nothing reaps it
/// (`docs/ROADMAP.md`, the receive-path ledger). Pre-existing; named here so
...
impl Drop for TcpConn {
    fn drop(&mut self) {
        ...
        // The session keys die with the connection too, and are OVERWRITTEN,
        // not merely dropped: the slot's bytes stay on `serve`'s stack until a
        // SYN rebuilds it, and a key left there authenticates the session's
        // traffic to anyone who later reads that memory.
...
/// A keyed session's state on the export: the two directions' keys, the last
/// request `seq` accepted, and WHO every verb on it runs as. The user is the
/// one who signed the `NP_SESSION`, resolved there once; a keyed frame carries
/// no name (Decision 4 of docs/roadmap/roadmap-session-auth.md). That is a
/// change and not a preservation: on an unkeyed session a path verb runs as
/// whatever name its own request signs.
struct KeyedSession {
    /// Verifies requests: `HMAC-SHA-512(k_c2s, seq ‖ NP message)`.
    k_c2s: [u8; ninep_abi::NP_SESSION_KEY_LEN],
    /// Tags replies: `HMAC-SHA-512(k_s2c, seq ‖ status ‖ result)`.
    k_s2c: [u8; ninep_abi::NP_SESSION_KEY_LEN],
    /// The last `seq` accepted; the next must be exactly one more. Starts at
    /// 0, so the first keyed request is 1.
    seq: u64,
    /// The session's user, mapped onto this machine's accounts.
    user: Proxy,
}

/// Overwrite a secret with zeros in a way the optimiser may not elide: the
/// buffer is about to be dropped or reused, which is exactly when a plain
/// store is dead and removed.
fn wipe(b: &mut [u8]) {
    for x in b.iter_mut() {
...
/// `NP_SESSION` is in flight (`Wire::held`).
const REMOTE_NP_MAX: usize = syscall_abi::MSG_MAX_LEN as usize - syscall_abi::NETOP_RMOUNT_MSG;
/// A remote slot's buffers. Out: a signed frame over `REMOTE_NP_MAX` (752)
/// bytes of NP is `NP_FRAME_MAX`-shaped: 908. In: a sealed reply to a
/// `NP_REMOTE_CHUNK` (512) read is under 600, and to a readdir filling the
/// client's whole 768-byte reply under 840. A framed reply that cannot fit is
/// a protocol error, failed at once rather than waited for. Sized to the byte
/// because every slot sits on `serve`'s fixed stack.
const REMOTE_SBUF: usize = ninep_abi::NP_NET_LEN_PREFIX + ninep_abi::NP_AUTH_HDR_SIGNED + REMOTE_NP_MAX;
const REMOTE_RBUF: usize = 1024;
/// Every parking slot across BOTH tables (`remotes`, then `sessions`), indexed
/// as `remote_src_port` numbers them. Each draws its source port from its own
/// disjoint `REMOTE_PORT_SPAN`-wide slice of the ephemeral window, so two live
/// parking slots never share a 4-tuple and their replies never cross: the
/// property `next_src_port` (unique only across calls a round trip apart) lost
/// when the park stopped waiting (review of #149). The window used to be split
...
// which is the peer's TIME_WAIT collision `next_src_port` exists to avoid.
const _: () = assert!(REMOTE_PORT_SPAN >= 0x400, "too many parking slots for the ephemeral window: each slot's slice is under 1024 ports");
...
/// Ticks of SILENCE a parked `cpu` run may take before it is given up on.
/// A run is bounded by silence rather than by total time: a remote command may
/// legitimately take a while, and every byte it sends resets this. Far above
/// `REMOTE_DEADLINE_TICKS`, which bounds a single request/reply round trip.
///
/// `tcp_run` used 300 ticks (6 s) of silence, but reset it on ANY frame it
/// served while waiting, including a Phase 4b child's `/host` callbacks to our
/// own export. A parked run never sees those (the serve loop serves them, on
/// other connections), so reproducing that coupling would mean threading
/// export activity into this slot. Thirty seconds instead: strictly MORE
/// permissive than the old rule for the case that changed (a child working
/// silently on `/host` reads), and a dead peer still costs one of `MAX_REMOTE`
/// slots for a bounded time rather than forever.
const RUN_DEADLINE_TICKS: u64 = 1500;
...
struct Parked {
    ...
    /// The tick the request's deadline clock started, for
    /// `REMOTE_DEADLINE_TICKS`: the first SYN send on a fresh connection, or
    /// the park itself on a session already open. 0 = not started.
    since: u64,
    /// The caller's name, kept for a session slot's second phase: the held verb
    /// is signed only once the far side has accepted the `NP_SESSION`.
    who: [u8; ninep_abi::NP_NAME_LEN],
    /// The NP verb in flight, for a session slot's fid refcount (`NP_OPEN`
    /// adds one, `NP_CLUNK` removes one). Read on a path slot too, where it is
    /// never a fid verb, so the count there stays 0 and the slot closes.
    verb: u64,
    /// Phase one of a session slot's two-phase park: the frame in flight is
...
fn sign_parked<const S: usize, const R: usize, const H: usize, const K: usize>(c: &mut Wire<S, R, H, K>, auth: &Auth) -> bool {
    ...
    if let Some(ka) = c.keys.first_mut() {
        if let KeyState::Keyed { k_c2s, seq, .. } = &mut ka.state {
            let total = frame_keyed_in_place(k_c2s, seq, NP_SIGNED_NP_OFF, pk.raw as usize, &mut c.sbuf);
            pk.raw = 0;
            if total == 0 {
                return false;
            }
        ...
        if pk.opening {
            if let (Some(id), Some(boot)) = (auth.signing.as_ref(), boot_counter()) {
                ...
                if at + ninep_abi::NP_EPHEMERAL_LEN <= c.sbuf.len() {
                    offer_key(ka, id, boot, &mut c.sbuf[at..at + ninep_abi::NP_EPHEMERAL_LEN]);
...
#[inline(never)]
fn offer_key(ka: &mut SessionAuth, id: &Identity, boot: u64, payload: &mut [u8]) {
    ka.forget();
    let eph = ephemeral_secret(ninep_abi::SIG_DOMAIN_EPHEMERAL_C, &id.seed, boot, &[]);
    let eph_pub = ed25519::x25519_public(&eph);
    payload.copy_from_slice(&eph_pub);
    ka.state = KeyState::Offered { eph, eph_pub };
}
...
/// Frame a raw NP request `AUTHNP04` in place: the request sits at `src` (a
/// parked one at `NP_SIGNED_NP_OFF`, the layout `park_raw` writes; phase two's
/// held verb already at `NP_KEYED_NP_OFF`), and moves to `NP_KEYED_NP_OFF` if
/// it is not there, with the header written in front of it. Spends the next `seq`, which is
...
#[inline(never)]
fn frame_keyed_in_place(k_c2s: &[u8; ninep_abi::NP_SESSION_KEY_LEN], last_seq: &mut u64, src: usize, np_len: usize, out: &mut [u8]) -> usize {
    let p = ninep_abi::NP_NET_LEN_PREFIX;
    if src.max(NP_KEYED_NP_OFF) + np_len > out.len() || np_len > ninep_abi::NP_NET_MAX {
        return 0;
    }
    let Some(seq) = last_seq.checked_add(1) else {
        return 0;
    };
    if src != NP_KEYED_NP_OFF {
        out.copy_within(src..src + np_len, NP_KEYED_NP_OFF);
    }
    let tag = keyed_tag(k_c2s, seq, &out[NP_KEYED_NP_OFF..NP_KEYED_NP_OFF + np_len]);
...
#[inline(never)]
fn verify_keyed_reply(resp: &[u8], got: usize, k_s2c: &[u8; ninep_abi::NP_SESSION_KEY_LEN], seq: u64) -> Option<(usize, usize)> {
    let p = ninep_abi::NP_NET_LEN_PREFIX;
    if got < p {
        return None;
    }
    ...
    let expect = keyed_tag(k_s2c, seq, &body[tl..]);
...
fn phase_two<const S: usize, const R: usize, const H: usize, const K: usize>(
    ...
    c.held_len = 0;
    // The four outcomes in ONE match, on what we offered and what came back
    // (review of step 7: spread over three conditions, relaxing one silently
    // changed which answers key; the export was reshaped the same way).
    let offered = c.keys.first().is_some_and(|a| matches!(a.state, KeyState::Offered { .. }));
    match (offered, result.len()) {
        // Nothing offered, nothing answered: a signed session, as before.
        (false, 0) => {}
        // A key we never offered, or an answer the wrong size for one.
        (false, _) => return Err(syscall_abi::FS_ERR_AUTH),
        (true, n) if n != 0 && n != ninep_abi::NP_EPHEMERAL_LEN => {
            if let Some(ka) = c.keys.first_mut() {
                ka.forget();
            }
            return Err(syscall_abi::FS_ERR_AUTH);
        }
        // Offered, and the export does not key: signed, and the secret goes now.
        (true, 0) => {
            if let Some(ka) = c.keys.first_mut() {
                ka.forget();
            }
        }
        // Offered, and answered with `eph_export`: keyed.
        (true, _) => {
            let Some(ka) = c.keys.first_mut() else {
                return Err(syscall_abi::FS_ERR_AUTH);
            };
            let KeyState::Offered { eph, eph_pub } = &ka.state else {
                return Err(syscall_abi::FS_ERR_AUTH);
            };
            let eph_pub = *eph_pub;
            let mut eph_export = [0u8; ninep_abi::NP_EPHEMERAL_LEN];
            eph_export.copy_from_slice(result);
            let shared = ed25519::x25519_shared(eph, &eph_export);
            ka.forget();
            ...
            ka.state = KeyState::Keyed { k_c2s: [0; ninep_abi::NP_SESSION_KEY_LEN], k_s2c: [0; ninep_abi::NP_SESSION_KEY_LEN], seq: 0 };
            let KeyState::Keyed { k_c2s, k_s2c, seq } = &mut ka.state else {
                unreachable!()
            };
            k_c2s.copy_from_slice(&k[..kl]);
            k_s2c.copy_from_slice(&k[kl..2 * kl]);
            wipe(&mut k);
            // The held verb, keyed: copied once, straight to where the keyed
            // frame carries it, and framed in place.
            if NP_KEYED_NP_OFF + held_len > c.sbuf.len() {
                return Err(syscall_abi::FS_ERR_AUTH);
            }
            c.sbuf[NP_KEYED_NP_OFF..NP_KEYED_NP_OFF + held_len].copy_from_slice(&c.held[..held_len]);
            let total = frame_keyed_in_place(k_c2s, seq, NP_KEYED_NP_OFF, held_len, &mut c.sbuf);
...
/// path `/net/tcp/N/...` (a [`DialConn`]), a remote-mount request parked on
/// its own connection (a [`RemoteConn`]), or a held session a remote fid's
/// verbs ride one after another (a [`SessionConn`]). Leaner than [`TcpConn`]
...
struct Wire<const S: usize, const R: usize, const H: usize, const K: usize> {
    ...
    /// A session slot's key, with `peer_ip`/`peer_port`: the LOCAL sender's uid
    /// (`SENDER_ID`), which selects the signing name and so the far-side
    /// identity every fid on this session is bound to. One user's fids to one
    /// endpoint share a session and its `SESSION_FIDS` budget; two users get
    /// two sessions. 0 on a dial and on a path slot.
    uid: u32,
    /// Open fids `netd` has relayed on this session. `NP_OPEN` success adds one,
    /// a successful `NP_CLUNK` removes one, and the session is closed when it
    /// reaches zero - the far export's own close-clunks-everything, seen from
    /// this side. A refcount, not a timer, so a live handle never vanishes
    /// under its holder (the reason Decision 3 rejected a policy lifetime).
    /// Always 0 on a dial and on a path slot, which is why those close after
    /// one reply.
    fids: u32,
    /// A session slot's first verb, held raw while the `NP_SESSION` that must
    /// precede it is in flight (`Parked::opening`), and signed from
    /// `service_remotes` once the far side has said yes: the far export serves
    /// one framed request per session reset, so the two cannot be pipelined.
...
/// One client session's authentication: the name it was opened as, and where
/// its keying stands. Wiped by `Drop`, which runs wherever the slot is released
/// (a refused or empty answer, an unreachable peer, a low-order close, a
/// reap), because zeroing on the success path alone would leave a secret
/// resident on exactly the paths nobody watches (Decision 6).
struct SessionAuth {
    /// The name the session's `NP_SESSION` was signed as, and so the far-side
    /// identity EVERY verb on it runs as once it is keyed (a keyed frame carries
    /// no name). A verb whose caller now resolves to another name (the uid's
    /// `/etc/passwd` line changed while the session was held) is refused rather
    /// than run as the opener: sessions are looked up by uid, the far side
    /// authorizes by name, and the two must not drift apart (review of step 7).
    opener: [u8; ninep_abi::NP_NAME_LEN],
    state: KeyState,
}

/// Where a client session's keying stands. One enum rather than a struct of
/// every field, because the handshake's state and the session's keys are
/// never live together: an enum holds the larger of the two, where fields
/// held both for the session's whole life, on `serve`'s frame, at the
/// client's deepest point (review of step 7: 144 bytes a slot, now ~80).
enum KeyState {
    /// No key offered, or the session is unkeyed: signed per request.
    None,
    /// The `NP_SESSION` in flight offered `eph_pub`; `eph` is its secret, kept
    /// until phase two derives the keys or the session is refused.
    Offered {
        eph: [u8; ed25519::X25519_LEN],
        eph_pub: [u8; ninep_abi::NP_EPHEMERAL_LEN],
    },
    /// Keyed: every request is `AUTHNP04` under `k_c2s`, every reply must carry
    /// a tag under `k_s2c`. `seq` is the last request's, which is the one in
    /// flight: a session has one verb in flight at a time, so a reply is
...
impl SessionAuth {
    const NONE: SessionAuth = SessionAuth { opener: [0; ninep_abi::NP_NAME_LEN], state: KeyState::None };

    /// Wipe whatever secret the state holds and return to `None`: at phase two
    /// once the ephemeral is spent, on every refusal of the opening, and in
    /// `Drop`.
    fn forget(&mut self) {
        match &mut self.state {
            KeyState::None => {}
            KeyState::Offered { eph, .. } => wipe(eph),
            KeyState::Keyed { k_c2s, k_s2c, .. } => {
                wipe(k_c2s);
                wipe(k_s2c);
            }
        }
        self.state = KeyState::None;
    }
}
...
fn new_wire<const S: usize, const R: usize, const H: usize, const K: usize>(src_port: u16) -> Wire<S, R, H, K> {
    Wire {
        ...
        held: [0; H],
        held_len: 0,
        keys: [SessionAuth::NONE; K],
    }
}

/// The source port for parking slot `index`, numbered across both tables
/// (`remotes` first, then `sessions` from `MAX_REMOTE`): its own slice of the
/// ephemeral window, varied within the slice by the clock so a slot's
/// back-to-back connections to one peer never land on the peer's lingering
/// TIME_WAIT socket. (A dial, or `tcp_get`/`tcp_run`, could still collide with
/// a parking slot - the pre-existing broad case `next_src_port` never fully
/// closed - which self-heals via the reply's nonce check and TCP retransmit.)
fn remote_src_port(index: usize) -> u16 {
    TCP_SRC_PORT + (index as u16) * REMOTE_PORT_SPAN + (now_us() % REMOTE_PORT_SPAN as u64) as u16
}
...
fn on_frame(mac: &[u8; 6], frame: &[u8], conns: &mut [Option<TcpConn>; MAX_CONNS], dials: &mut [Option<DialConn>; MAX_DIAL], remotes: &mut [Option<RemoteConn>; MAX_REMOTE], sessions: &mut [Option<SessionConn>; MAX_CLIENT_SESSIONS], auth: &Auth) {
    if frame.len() < 14 {
        return;
    }
    ...
    match ethertype {
        ...
        0x0800 => {
            // A client connection's segment (a parked remote mount's, a held
            // session's, or a dial-out's) is addressed to our ephemeral source
            // port, not a listen port - try those first (and a retransmitted
            // SYN for an already-accepted dial-in conn matches here too, so it
            // never double-accepts below).
            if dial_on_segment(mac, frame, remotes) {
                return;
            }
            if dial_on_segment(mac, frame, sessions) {
                return;
            }
...
fn handle_tcp(mac: &[u8; 6], frame: &[u8], seg: &TcpIn, conns: &mut [Option<TcpConn>; MAX_CONNS], dials: &mut [Option<DialConn>; MAX_DIAL], auth: &Auth) {
    ...
    if seg.flags & TCP_SYN != 0 && seg.flags & TCP_ACK == 0 {
        ...
        let mut c = TcpConn {
            ...
            cpu_child: CPU_NONE,
            session: false,
            fids: [SessionFid::NONE; SESSION_FIDS],
            keyed: None,
            carried: false,
            last_rx: now(),
    ...
    if !c.responded && seg.data_len > 0 && seg.seq == c.rcv_nxt {
        ...
        if c.local_port == ninep_abi::NP_NET_PORT {
            if handle_9p(c, request, dials, mac, auth, sessions) {
                ...
                // parked verb ends now instead of waiting on a reply that will
                // never come; the slot, and with it every fid and the session
                // keys, is freed by the caller.
                send_seg(mac, c, TCP_RST | TCP_ACK, false, &[]);
                return true;
            }
        } else {
...
///
/// Returns `true` when the connection must be CLOSED WITHOUT A REPLY: an
...
/// store held a counter from before this boot). A node with none keys no
/// sessions (Decision 6 of docs/roadmap/roadmap-session-auth.md).
fn boot_counter() -> Option<u64> {
...
fn key_session(
    ...
    // eph_export): the nonce and both public keys bind the keys to THIS
    // handshake. k_c2s is the first half, k_s2c the second.
    let mut h = ed25519::Sha512::new();
    h.update(ninep_abi::SIG_DOMAIN_SESSION);
    h.update(&shared);
    h.update(nonce);
...
/// previous life, step 3's review), and the two entropy sources are what give
...
fn ephemeral_secret(domain: &[u8], seed: &[u8; ed25519::SECRET_LEN], boot: u64, nonce: &[u8]) -> [u8; ed25519::X25519_LEN] {
    ...
    wipe(&mut e);
    // Hardware entropy, absent on Parallels and the Pi: a short read is used
    // for what it is, since every byte the device gave is still entropy.
    let mut r = [0u8; 32];
    let n = syscall4(syscall_abi::RANDOM, r.as_mut_ptr() as u64, r.len() as u64, 0, 0);
    if n != syscall_abi::RANDOM_UNAVAILABLE {
        h.update(&r[..(n as usize).min(r.len())]);
    }
    wipe(&mut r);
    let mut d = h.finalize();
    let mut secret = [0u8; ed25519::X25519_LEN];
    secret.copy_from_slice(&d[..ed25519::X25519_LEN]);
    wipe(&mut d);
    secret
}
...
fn keyed_tag(key: &[u8; ninep_abi::NP_SESSION_KEY_LEN], seq: u64, data: &[u8]) -> [u8; ninep_abi::NP_KEYED_TAG_LEN] {
    ...
    m.update(data);
    let full = m.finalize();
    let mut t = [0u8; ninep_abi::NP_KEYED_TAG_LEN];
    t.copy_from_slice(&full[..ninep_abi::NP_KEYED_TAG_LEN]);
    t
}
...
/// request's and is not sent again. Returns the new framed length.
///
...
fn seal_keyed(c: &mut TcpConn, seq: u64, n: usize) -> usize {
    ...
    let Some(k) = c.keyed.as_ref() else {
        return frame_reply(&mut c.prefix, syscall_abi::FS_ERR_AUTH, &[]); // unreachable
    };
    let tag = keyed_tag(&k.k_s2c, seq, &c.prefix[body_start..n]);
    c.prefix.copy_within(body_start..n, body_start + tl);
    c.prefix[body_start..body_start + tl].copy_from_slice(&tag);
    let new_body = n - body_start + tl;
...
/// the FIN while Closing, and reap idle slots. Called each `serve` pass, once
/// per table. `idle_ticks` is that table's idle reap: `DIAL_IDLE_TICKS` for a
...
/// What a request's words mean, per verb: the ONE place the export says which
/// verbs carry a path and where, which name a fid, and which it does not serve.
...
enum Shape {
    ...
    Path,
    /// `NP_OPEN`: `a0` = the `OPEN_*` flags, `a1` = the path length - the
    /// reverse of every other path verb (step 2's trap, found by the host
    /// peer's self-test). Needs a session, since it makes a fid.
    Open,
    /// An op on a fid this session holds: `a0` = the fid, no path at all. The
    /// namespace is not consulted: the fid's tree and path are fsd's, recorded
...
fn build_9p_reply(
    ...
    let shape = shape(verb);
    // The fid verbs live on a SESSION only: on a one-shot connection the fid
    ...
    // the export had a fid table.
    if matches!(shape, Shape::Open | Shape::Fid) && !session {
    ...
    match shape {
        ...
        // copied path arm could have used by accident - review of the step-6
        // PR). `session_slot` is the shared front half: the slot behind the
        // client's number, or the refusal.
        Shape::Fid => {
            ...
            return match verb {
                // Read through a fid: a1 = offset, a2 = count. fsd's NP_PREAD
                // answers INLINE (the bytes ride in its reply, as NP_READDIR's
                // do), not through a grant like the path-based NP_READ that
                // read_file_chunk bridges, so the relay is one fsd_call3 and
                // the cap is fsd's inline reply size, the same DATA_INLINE the
                // readdir arm uses. Status = the bytes read (0 at EOF), relayed
                // as fsd answers it: a short read is the client's to continue
                // from, exactly as libc's read() loop does locally. No per-op
                // permission check: the fid was authorized at open, and fsd
                // refuses a read on a fid not opened for it.
                ...
                v if v == ninep_abi::NP_FSTAT => {
                    let status = fsd_call(who, v, 0, fsd_fid, 0, &[], &mut buf[..ninep_abi::STAT_INFO_LEN]);
                    frame_reply(out, status, ok_data(status, &buf))
                }
                // Close a fid. Freed whatever fsd answers, with one exception.
                // A fid fsd no longer knows (it restarted) is gone either way,
                // and a slot kept for it would never free. But fsd REFUSES a
                // clunk from a user other than the opener and keeps the fid
                // (its `Fid::owner_uid`), so on that answer the slot is kept
                // too: clearing it would orphan a live fid on fsd under netd's
                // own identity, which nothing could ever reap, and kill the
                // owner's handle on this side. Unreachable past session_slot's
                // uid check; that check is what the export's mutation control
                // removes, and this is what makes that run honest (review of
                // #130).
                v if v == ninep_abi::NP_CLUNK => {
                    let status = fsd_call(who, v, 0, fsd_fid, 0, &[], &mut []);
    ...
    let plen = if shape == Shape::Open { p1 as usize } else { p0 };
    let mut pbuf = [0u8; 256];
    let (tree, fspath): (u64, &[u8]) = {
        let path = &payload[..plen.min(payload.len())];
        let resolved = ninep_abi::resolve_ns(EXPORT_NS, path, &mut pbuf);
        // An open of anything but an fsd tree is refused HERE, before the
        // target's own arm sees it: the console and /net have no fid model
        // (here or locally - libc refuses them the same way), and a remote
        // binding would be transitive mounting, which the export does not
        // do. It has to be here and not in those arms, because the /net arm
        // hands its whole /tcp subtree to the dial code before looking at
        // the verb, and an NP_OPEN that reached it was answered by the
        // clone/ctl/data logic with a bare status and no log line (the
        // first fold did exactly that; found by review, pinned by the
        // path gate's non-fsd opens).
        if shape == Shape::Open && !matches!(resolved.target, ninep_abi::NsTarget::Fsd(_)) {
    ...
    match verb {
        ...
        // --- NP_OPEN (roadmap-fid-verbs.md step 5): the one fid verb with a
        // path, resolved above like any other path verb's (a1 bytes; a0 = the
        // OPEN_* flags), and refused above for any target but an fsd tree.
        // The ops on the fid it makes are dispatched before resolution. ---
...
/// reach netd there at all. Step 5 of `docs/roadmap/roadmap-fid-verbs.md` gave
/// `TcpConn` netd-owned per-connection state (the fid table), so a
/// per-connection cap now has somewhere to live; it is still not built, and
/// the exposure above is unchanged.
