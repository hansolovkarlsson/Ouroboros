...
fn main() -> ! {
    ...
    // The proxy-identity latch (cluster per-user identity): set by NP_PROXY_ID
    // from netd, consumed by exactly the next request, and never a static -
    // this server keeps all its state on this frame, deliberately.
    let mut proxy: Option<Caller> = None;
    loop {
        ...
        let reply_len = handle(&mut mounts, &mut fids, sender, &req[..len], &mut reply, &mut proxy);
...
/// in the reply).
fn handle(mounts: &mut [Option<vfs::Filesystem>; MAX_MOUNTS], fids: &mut [Fid; MAX_FIDS], sender: u64, req: &[u8], reply: &mut [u8], proxy: &mut Option<Caller>) -> usize {
    if req.len() < REQ_PAYLOAD {
        return status_reply(reply, syscall_abi::FS_ERROR);
    }
    let op = read_u64(req, 0);

    // NP_PROXY_ID: "run my next request as this user". The ONE verb that takes
    // a claimed identity, and only from NET_TASK - see the ABI doc for why the
    // exception must exist (netd forwards remote requests, so without it every
    // one of them is served under netd's own root) and why it is safe.
    //
    // Checking the SLOT here, not a credential, is deliberate and sound: slots
    // below FIRST_SPAWNABLE are protected, so NET_TASK is netd for the life of
    // the boot and can never be recycled to anything else. That is a different
    // question from "who sent this", which is why it is answered differently.
    if op == ninep_abi::NP_PROXY_ID {
        if sender != syscall_abi::NET_TASK {
            *proxy = None;
            return status_reply(reply, syscall_abi::FS_ERR_PERM);
        }
        let uid = read_u64(req, 16) as u32;
        let gid = read_u64(req, 24) as u32;
        let n = (read_u64(req, 32) as usize).min(syscall_abi::MAX_SUPP_GROUPS);
        let mut groups = [0u32; syscall_abi::MAX_SUPP_GROUPS];
        let body = &req[REQ_PAYLOAD..];
        let n = n.min(body.len() / 4);
        for (i, g) in groups.iter_mut().enumerate().take(n) {
            *g = u32::from_le_bytes([body[i * 4], body[i * 4 + 1], body[i * 4 + 2], body[i * 4 + 3]]);
        }
        *proxy = Some(Caller { uid, gid, groups, n_groups: n });
        return status_reply(reply, 0);
    }

    // The caller, resolved ONCE per request rather than re-read at each check -
    // so every check in this request sees the same identity, and the latch is
    // consumed by exactly the request that follows it and never the one after.
    let who = effective_caller(sender, proxy);
    *proxy = None;
...
/// control ops, which never pass through [`check_access`]. A caller the kernel
/// has no captured credential for is not root.
fn is_root(who: &Option<Caller>) -> bool {
    matches!(who, Some(c) if c.uid == 0)
}

/// The identity this request is authorized against: a proxy identity when
/// `netd` set one for it, otherwise the credential the kernel bound at send.
///
/// The proxy is *peeked*, not consumed - `handle` clears it once, after
/// resolving, so every check within one request sees one identity and the
/// latch can never carry into a later request.
fn effective_caller(sender: u64, proxy: &Option<Caller>) -> Option<Caller> {
    if sender == syscall_abi::NET_TASK {
        if let Some(c) = proxy {
            return Some(*c);
        }
