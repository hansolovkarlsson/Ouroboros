...
## Network stack, Stage 4c: a static-file HTTP server - netd becomes fsd's first non-shell client

Stage 4b's server answered *every* request with one fixed page. This makes
it a real static-file server: it parses the request-target path and, for
anything but `/`, streams that file from the filesystem server over TCP.
The guest serves its own filesystem - `curl localhost:5555/EFI/ORBS/INIT.CFG`
returns the file's actual bytes.

**The architecturally interesting part isn't the parsing - it's that `netd`
becomes `fsd`'s first non-shell client.** Until now the shell was the *only*
task that called the filesystem server. A request now arrives at `netd` over
TCP -> `netd` `MSG_CALL`s `fsd` to read the file -> `netd` streams it back
over TCP. Two userland servers cooperating to answer one external request -
the microkernel driver-isolation design actually paying off, not just
described. One deliberate, **visible** capability change makes it legal:
`netd`'s send-mask (`caps_for_slot`, `NET_TASK` arm) gains `TO_FSD`. That's
exactly the kind of cross-server edge the capability model exists to make
explicit rather than ambient - before this, `netd` could only reach `cond`
(logging). No deadlock risk: `fsd` never calls `netd`, so the call graph
stays acyclic.

**The file read reuses the shell's exact machinery, ported.** `read_file_chunk`
`GRANT`s a `GRANT_WRITE` buffer to `fsd` and issues `FSOP_READ_BULK`; the
server `SAFECOPY`s the bytes straight in - the same grant/safecopy bulk path
`cat` streams over, just living in `netd` now (the shell's `fs_read_bulk`,
essentially verbatim). `serve_http` parses the path (`GET /path`, query
string stripped, no %-decoding), serves a built-in landing page for `/`,
else probes the file with the first chunk (so a 404/503 *replaces* the 200
header rather than following it), then streams: a 200 header segment, then
the body in `SERVE_CHUNK` (1400)-byte `PSH|ACK` segments - one per
`FSOP_READ_BULK`, each under the MSS, `snd_nxt` advanced past each - until
EOF or the size cap. A missing file is a 404, a missing/unmounted filesystem
(`TASK_ERR_NO_SUCH_TASK` back from the call) a 503.

**Bounded at `MAX_SERVE` (~16 KB), deliberately.** The server blasts body
segments without waiting for ACKs (no flow control yet), so the whole body
must fit inside the peer's advertised window (~64 KB); 16 KB stays
comfortably under it. A larger file is truncated at the next chunk boundary
past 16 KB - honest and safe, not silently corrupt. Real flow control
(pace the body against incoming ACKs) is the follow-up that lifts this.

**Confirmed on QEMU (`make run-image-server`), zero `-d int` aborts:**
`curl localhost:5555/` -> the landing page; `curl .../EFI/ORBS/INIT.CFG` ->
`HTTP/1.0 200 OK` + the file's real content (`\EFI\ORBS\SH.BIN`), read
through `fsd`; `curl .../nope/missing` -> `404 Not Found`; and
`curl .../EFI/ORBS/SH.BIN` (a 72 KB file) -> the first 16800 bytes
(12×1400), **byte-identical to the real file**, streamed over 12 body
segments plus the header - the multi-segment path proven, not just reasoned.
The shell (fsd's other client), `selftest`, and the client net ops
(`ping`/`resolve`) all stayed unregressed with `netd` now also calling `fsd`.

**Still coarse, worth knowing before building on this:** the ~16 KB cap
**(Update: lifted - Stage 4d, flow control)**; no `Content-Length`
...
Stage 4c capped a served file at `MAX_SERVE` (~16 KB) because it blasted the
whole body in one shot, which only works if it fits the peer's receive
window. This adds real **send-side flow control**: the server never lets
unacknowledged data (`snd_nxt - snd_una`) exceed the peer's advertised
window, and streams the body *paced by the peer's ACKs* - so a file of any
size flows, bounded by the client's consumption instead of one window. The
cap is gone.

**The TCP server became stateful across event-loop wakes** - the real change
in shape. `TcpConn` now tracks `snd_una` (advanced by the peer's ACKs), the
peer's `window`, and the in-progress response (a fixed prefix - the 200
header, or a whole fixed/404/503 body - then a file body streamed from `fsd`
by rising offset). `TcpIn` parses the peer's `ack`+`window`; `handle_tcp`
updates `snd_una`/`window` on every segment (`seq_gt` handles the u32
sequence wraparound) but no longer *sends* inline. `pump_send` sends one
window-and-MSS-bounded segment; `serve()` loops `pump_send` + a mailbox
drain after the frame batch, flushing a full window per wake, then blocks
in `NET_WAIT` again - the peer's next ACK (freeing window) drives the next
round.

**The per-segment mailbox drain is load-bearing, and it's the subtle part.**
A large transfer is many slow `fsd` reads (each a 1400-byte chunk = an IPC
round trip + a virtio-blk read, ~tens of ms under QEMU's TCG), so netd is
busy for seconds. The supervisor's active health-ping must be acked within
~160 ms or netd is judged wedged and **restarted mid-transfer** (losing the
connection state on its stack - observed directly, twice, as a stalled
truncated download). Draining the mailbox (acking the ping) after *every
segment* - not every burst - keeps the worst ack latency to one segment
(~a few ms), far under the timeout. Bursts of 16 and 48 were each measured
to occasionally overrun it under TCG's variable latency; per-segment
draining removes the race entirely at negligible cost (one extra
non-blocking syscall per segment, dwarfed by the disk read). The window
still bounds correctness (never overrun the peer); the per-segment drain
bounds responsiveness.

**A real stack overflow the guard page caught, fixed by growing the stack.**
The flow-control change added one frame (`drain_client_messages`) to netd's
client-op path (`ping`/`resolve`/`fetch`) - which already nested several
1600/2048-byte frame buffers down one call chain (`serve` -> `handle_*` ->
`arp`/`tcp`/`dns`) and sat right at the 16 KB stack edge. `ping` tipped
~544 bytes into netd's guard page: a clean, contained EL0 permission fault
(`esr=0x9200004f`, `far` in the guard page), reported and the task killed/
restarted - *exactly what the guard page milestone built the guard for*, not
a silent corruption. netd is the most stack-hungry userland program (a full
TCP client+server + DNS + the file server); `STACK_PAGES` grew 4 -> 6 (16 KB
-> 24 KB per region, `loader.rs` + `mmu.rs` kept equal) to give it real
margin. Every region grows 8 KB - negligible.

**Confirmed on QEMU:** a **256 KB** file (well past any single window)
streams byte-identical **three times in a row**, zero supervisor restarts,
zero faults; plus the 60 KB `SH.BIN`, the index/`INIT.CFG`/404 responses,
and a full shell+disk+client-net regression (`selftest`, `ls`/`cat`/`write`,
`ping`/`resolve`/`fetch`) - zero `-d int` aborts throughout. Throughput is
disk-bound (~55 KB/s under TCG: each chunk is an `fsd` IPC round trip + a
virtio-blk read), **not** a TCP-layer limit - and the network stack is
QEMU-only regardless (Parallels' virtio-net is PCI).

**Still coarse, worth knowing before building on this:** no retransmission
(a lost segment on a real lossy path would stall - fine on SLIRP's reliable
loopback, a real gap on a real network); no congestion control (only the
peer's flow-control window, no `cwnd`/slow-start); one connection at a time
(inherited); no window scaling (a >64 KB window isn't negotiated - fine, the
peer's ~64 KB is plenty); still QEMU-only, polled RX.

## Network stack, Stage 4e: proper HTTP response headers (Content-Type + Content-Length)

A small, self-contained polish over the file server: it sent every file as
`application/octet-stream` with no length, so browsers *downloaded* instead
of rendering and clients relied on connection-close to find the end. This
makes the responses correct - pure `netd`, no kernel/capability/state-machine
change.

- `start_response` now **stats** the file (one `FSOP_READ_FILE` whose status
  is the file's *real* size - `want=1`, the smallest fsd accepts since its
  `want_len` rejects 0; the single returned byte is ignored) instead of a
  probe read. The stat both checks existence (404/503 as before) and gives
  the size for `Content-Length`; the body still streams from offset 0.
- `content_type()` picks a MIME type from the path extension
  (case-insensitive): `html`/`htm`, `txt`/`cfg`/`md`/`log`, `css`, `js`,
  `json`, `png`, `jpg`/`jpeg`, `gif`; everything else stays
  `application/octet-stream`.
- `build_200_header()` formats the `200 OK` with `Content-Type` +
  `Content-Length` (`u64_decimal`, hand-rolled - `netd` builds all its bytes
  manually). `TcpConn`'s `prefix` became an owned `[u8; PREFIX_MAX]` buffer
  instead of a `&'static` slice so the header can be built per-request;
  `INDEX_PAGE`/404/503 are copied in via `set_prefix`.

**Confirmed on QEMU** (`curl -i`): `INIT.CFG` (`.cfg`) -> `text/plain`,
`Content-Length: 16`; `PAGE.HTM` (`.htm`) -> `text/html`, `Content-Length:
63` (renders in a browser now); `SH.BIN` (`.bin`) ->
`application/octet-stream`, `Content-Length: 59968`; a 256 KB file ->
`Content-Length: 262144` with a byte-identical streamed body (proper headers
and flow control together); 404 for a missing path. One honest note that is
*not* a bug: a `.html` (4-char extension) name 404s because it isn't a valid
FAT 8.3 short name and `fsd` has no long-filename support - the documented
no-LFN limitation; a `.htm` name works. Shell + client net ops
(`ping`/`resolve`/`fetch`) + `selftest` unregressed; zero `-d int` aborts,
zero supervisor restarts.

**Still coarse:** `Content-Type` is a small fixed extension map (no config,
