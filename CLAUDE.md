...
fsd/                 fifth userland program: THE FILESYSTEM SERVER (driver isolation part 2) - owns the FAT32 engine, speaks the FSOP_* protocol to clients over IPC (including the bulk FSOP_READ_BULK/FSOP_WRITE_BULK ops that move file data via grant/safecopy - see "Grant/safecopy IPC" above), and is the only task the BLOCK_* syscalls accept; boot-loaded into protected task slot 2, see "Driver isolation, part 2" above
  src/main.rs        the request loop: recv -> decode FsRequest -> dispatch to the mounted vfs::Filesystem -> reply one u64; it lives in main's stack frame (no static state in userland)
  src/vfs.rs         the internal filesystem-multiplexing layer (the VFS refactor): a Filesystem enum (Fat32 | ExFat) whose per-op methods forward to the arm, plus mount() which discovers partitions (partition.rs) and probes each FAT32-then-exFAT (first that validates wins), and name() for the mount log. An enum not dyn Trait (no_std, no heap - the block::BlockDevice/console::Console pattern); a further format (ext2) is a new arm + a probe/branch in mount. Clients never see it - the FSOP_* protocol is already FS-agnostic
  src/exfat.rs       exFAT read-only (the more-filesystems arc, step 2; the first real use of the Filesystem enum's second arm): mount_at parses the log2-shift boot sector, walk_dir reassembles directory *entry sets* (0x85 File + 0xC0 Stream-Ext + 0xC1 File-Name) into a DirEntry, advance() handles contiguous (NoFatChain, skips the FAT) vs FAT-chained clusters, UTF-16 names rendered ASCII; allocation bitmap (0x81) ignored (a write concern) and up-case table (0x82) approximated by ASCII case-fold. Read machinery mirrors fat32.rs; every write op returns Error::ReadOnly -> FS_ERR_READ_ONLY. See "More filesystems, step 2" in docs/CHANGELOG.md
...
Twenty-nine-crate workspace (`kernel`, `shell`, `hello`, `pong`, `fsd`,
`upper`, `cond`, `netd`, `args`, `ulib`, `echo`, `uptime`, `clear`, `ls`,
`cat`, `mkdir`, `rmdir`, `touch`, `rm`, `cp`, `mv`, `writeat`, `ping`, `resolve`, `fetch`, `wc`, `grep`, `head`, `syscall-abi`), with every userland crate deliberately excluded from the
