...
programs/fileutils/ (ls cat mkdir rmdir touch rm cp mv writeat tree write), programs/netutils/ (ping resolve fetch dial serve), programs/shellutils/ (echo uptime clear pwd readkey + args above; readkey is an interactive keyboard-echo diagnostic - the proof a /bin program can read the keyboard, see "foreground programs own the keyboard" below)  the externalized commands (Stage 4): former shell builtins, now real /bin programs (/bin/ECHO, /bin/LS, /bin/PING, ...) over ulib, found by PATH. echo/uptime/clear need neither fsd nor cwd; the filesystem commands (ls/cat + mkdir/rmdir/touch/rm + cp/mv/writeat + tree - ls sorts by name and lays out columns by default, `-l` is the long form (type/size/date-time via the NP_STAT op + ulib::fs_stat), `-a` shows dotfiles; tree is a recursive directory listing, sorted alphabetically, depth-capped for the 32KB spawn stack, ASCII branches since the framebuffer font is ASCII-only) resolve paths against the cwd the shell delivers at spawn (CWD_STAGE/GET_CWD) and talk to fsd via ulib's fs helpers; the network commands (ping/resolve/fetch) reach netd via ulib::net_call, using the TO_NET capability the shell delegates to them at spawn (a spawnable slot doesn't hold it statically). The whole fs+net command surface lives here now (pwd and write joined it - a spawned program can read the cwd + write files); only cd/bind (mutate the shell's OWN cwd/namespace) + exec/exit + shutdown/halt (power control, POWER syscall) + more/less (the pager - needs the keyboard) and job control (ps/kill/wait/fg) + mount/erase/partition/format (disk mgmt - must run with nothing mounted) + selftest/help + env/set/unset + cpu (remote execution - cluster Phase 4a) stay builtin
...
Forty-crate workspace. `kernel` and the two shared libs (`ulib`,
