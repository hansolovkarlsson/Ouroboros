...
upper/               sixth userland program: the pipeline filter demo (uppercases its piped input) - the reference for a *chainable* filter over ulib: stdin = msg_recv, stdout = write_out(stdout_target) so it can be a middle stage of a longer pipe (not a hardcoded console), EOF in = the empty message, EOF out = end_of_stream, finish = exit; see the multi-stage-pipeline arc in docs/roadmap.md
...
Twenty-six-crate workspace (`kernel`, `shell`, `hello`, `pong`, `fsd`,
`upper`, `cond`, `netd`, `args`, `ulib`, `echo`, `uptime`, `clear`, `ls`,
`cat`, `mkdir`, `rmdir`, `touch`, `rm`, `cp`, `mv`, `writeat`, `ping`, `resolve`, `fetch`, `syscall-abi`), with every userland crate deliberately excluded from the
