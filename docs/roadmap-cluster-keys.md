...
| 5 ✅ | It runs on the **guest** (`/bin/edtest`) | same vectors pass on-device; **3,392 B stack, ~1.4 ms sign+verify**; zero `ABS64` *and* zero `RELATIVE` in the linked binary | probe calibrated against a known 4 KB frame, so a silently-dead measurement cannot pass |
