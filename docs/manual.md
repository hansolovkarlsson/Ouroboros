...
shell if that task is gone; **Ctrl+C** terminates a foreground *command* the
owner is blocked waiting on, but merely **detaches** a nested shell handed the
keyboard with `fg` (the keyboard reverts to its parent, the shell survives with
its login and cwd, and `fg` resumes it), and the boot shell ignores it). An
...
| 3 | `try_read_char` | none | Non-blocking read; `NO_CHAR` if nothing waiting or the caller does not own the keyboard |
| 4 | `putc` | byte | Raw console byte write |
| 5 | *(gap)* | | *Deliberate gap (removed `shell_input`; ABI stability over density)* |
| 6 | `get_ticks` | none | Preemption tick count since boot |
| 7–14 | *(gap)* | | *Deliberate gaps: the old `fs_*` syscalls: the filesystem lives in userland now (the fsd server); their contracts survive as the `FSOP_*` request protocol below* |
| 15 | `read_char` | none | Blocking read: the task is suspended until a byte arrives and it owns the keyboard |
...
| 20 | `fg` | index, foreground | Hand keyboard ownership to a task, **caller must currently own the keyboard** (`foreground` = 1 for a command the caller waits on, 0 for a `fg`-handed session) (on the owner's death it reverts to the task that held it when `fg` handed it over, if still alive, else to task 0; Ctrl+C terminates a foreground command its parent is waiting on, but detaches a `fg`-handed session back to that parent without killing it) |
