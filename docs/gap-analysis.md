...
| Cluster authentication | ◐ | **Per-machine keypairs + per-user identity**: each machine holds an Ed25519 key and authorizes peers by public key (`/etc/cluster/authorized`, the `known_hosts` model — revoke by deleting a line), mutually authenticated (both request and reply signed, domain-separated) on the 9P export, fail-closed, `\NOEXEC` — and, since 2026-08-31, the requesting user's **name** inside the signature, resolved through the far side's own `/etc/passwd` (a stranger is refused) and carried into `fsd` as a required request field, so remote file access and `cpu` run as that user rather than root. **No per-user *keys*** (an authorized machine can claim any user with an account on the export, root included; planned in `roadmap/roadmap-user-keys.md`), **no replay protection, no on-the-wire encryption** — each a named next tier. Trusted-LAN by design. |
| Cryptography | ◐ | Hand-rolled SHA-256 (NIST-validated) for password hashing, and hand-rolled Ed25519 (RFC 8032 vectors) signing every cluster request and reply. No TLS, no key exchange, no at-rest encryption. |
...
| Config files (`/etc`) | ✗ | Only `INIT.CFG` (which program to load) and `/etc/cluster/*`; no general config layer. |
...
1. **The users/permissions arc — DONE, refinements included (2026-08-28 →
   ...
   owner/mode, root bypass, `FS_ERR_PERM`). Every refinement then listed here as
   remaining has since shipped too: ancestor-directory `x` traversal,
   `/etc/shadow`, the account tools, per-user homes under `/Users`, groups
   *including* supplementary membership, and — 2026-08-30 — self-service
