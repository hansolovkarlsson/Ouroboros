...
| Cluster authentication | ◐ | **Per-machine keypairs + per-user identity**: each machine holds an Ed25519 key and authorizes peers by public key (`/etc/cluster/authorized`, the `known_hosts` model — revoke by deleting a line), mutually authenticated (both request and reply signed, domain-separated) on the 9P export, fail-closed, `\NOEXEC` — and, since 2026-08-31, the requesting user's **name** inside the signature, resolved through the far side's own `/etc/passwd` (a stranger is refused) and carried into `fsd` as a required request field, so remote file access and `cpu` run as that user rather than root. **No per-user *keys*** (an authorized machine can claim any of its own users' names), **no replay protection, no on-the-wire encryption** — each a named next tier. Trusted-LAN by design. |
