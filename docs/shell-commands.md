...
| `partition` | `partition [fat32\|exfat\|ext2]` | **Destructive.** Writes a fresh MBR with one primary partition spanning the disk from LBA 2048 to the end, tagged with the given type (default `fat32` → 0x0C; `exfat` → 0x07; `ext2`/`linux` → 0x83) (`FSOP_PARTITION`). | Only LBA 0 (the partition table) is written; the partition is left **unformatted** — `format` (a later milestone) lays a filesystem into it, and `mount` won't succeed until then. Refused while mounted. GPT is a later step. A builtin, same reason as `erase`. Disk-management arc, milestone 2. |
