...
| `fg` | `fg <n>` | Hands the keyboard to task `n`, e.g. `exec /EFI/ORBS/SH.BIN` then `fg <n>` gives a nested shell session (its `exit` only logs out; end it with Ctrl+C). **Caveat**: today the nested shell keeps the keyboard only until the first command it runs exits, because ownership then reverts to the boot shell rather than to the spawner (an open item in `ROADMAP.md`). In this `exec` then `fg` flow the boot shell was at its prompt, so its read simply resumes and Ctrl+C is an ordinary ignored byte; `kill <n>` from the boot shell ends the nested shell. (Run `/EFI/ORBS/SH.BIN` as a foreground command instead and the boot shell is in a `wait`, which Ctrl+C then interrupts.) | **Ctrl+C is the escape hatch**: typed while another task owns the keyboard, the kernel terminates that task (the foreground task is killed, exactly as `kill` would; its slot becomes spawnable again) and ownership reverts to this shell. Ownership also reverts automatically when the foregrounded task exits or is killed. While this shell owns the keyboard, Ctrl+C (like every unhandled control byte) is ignored by the line editor. |
