...
| `kill` | `kill <n>` | Destroys task `n` (see `ps` for numbers). | The permanent slots (the boot shell, idle and the servers, every slot below the first spawnable one) are protected, and so is the shell's own slot: a child shell is ended with Ctrl+C, which terminates the foreground task, or with `kill <n>` from its parent shell; its `exit` only logs out and leaves the slot alive. A killed task's slot becomes spawnable again and its memory is reclaimed when allocation order allows. |
| `fg` | `fg <n>` | Hands the keyboard to task `n`, e.g. `exec /EFI/ORBS/SH.BIN` then `fg <n>` gives a nested shell session (its `exit` only logs out; end it with Ctrl+C, which kills it and returns the keyboard here). The nested shell keeps the keyboard across the commands it runs: when one of them dies, ownership returns to the task that held the keyboard when it was foregrounded, so the boot shell only gets it back when the nested shell itself dies. (Run `/EFI/ORBS/SH.BIN` as a foreground command instead and the boot shell is in a `wait` on it until then.) | **Ctrl+C is the escape hatch**: typed while another task owns the keyboard, the kernel terminates that task (the foreground task is killed, exactly as `kill` would; its slot becomes spawnable again) and ownership reverts to the task that held it when the dead task was foregrounded: this shell, for a command it ran. Ownership reverts the same way when the foregrounded task exits. While this shell owns the keyboard, Ctrl+C (like every unhandled control byte) is ignored by the line editor. |
