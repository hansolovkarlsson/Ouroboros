...
| `kill` | `kill <n>` | Destroys task `n` (see `ps` for numbers). | Tasks 0 (the boot shell), 1 (idle) and 2-5 (the servers) are protected, and so is the shell's own slot (a task ends itself with `exit`). A killed task's slot becomes spawnable again and its memory is reclaimed when allocation order allows. |
