...
| `format` | `format [fat32\|exfat]` | **Destructive.** Lays a fresh filesystem into the disk's first partition (`FSOP_FORMAT`) — mkfs. **FAT32 and exFAT** so far (`ext2` is a later step and reports "not yet"). | The partition must already exist (`partition` first); refused while mounted. After it succeeds, `mount -a` then `ls` shows an empty root. Validated against macOS `fsck_msdos` / `fsck_exfat`. A builtin, same reason as `erase`/`partition`. Disk-management arc, milestone 3. The typical prep flow is `unmount` → `erase disk` → `partition exfat` → `format exfat` → `mount -a`. |
