...
| `kill` | `kill <n>` | Destroys task `n` (see `ps` for numbers). | Tasks 0 (the boot shell), 1 (idle) and 2-5 (the servers) are protected, and so is the shell's own slot: a child shell is killed from its parent shell, after Ctrl+C returns the keyboard there, since `exit` only logs out and leaves the slot alive. A killed task's slot becomes spawnable again and its memory is reclaimed when allocation order allows. |
