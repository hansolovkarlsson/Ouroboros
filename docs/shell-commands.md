...
| `su` | `su <uid>[:<gid>]` | Changes this session's **user identity** (uid and/or gid; numeric — no `/etc/passwd` yet). Forms are chown-style: `uid`, `uid:gid`, `:gid`. | A **builtin** because `SET_ID` acts on the caller, so it must change the shell's *own* task identity (a `/bin` program would only change its short-lived child); commands spawned afterward inherit it (`id` proves it), and the prompt switches `#`↔`$`. **Only root may change identity** — a non-root session is refused (no escalation; there's no way back without login-style auth). See `man su`. |
