...
| `mv` | `mv <src> <dst>` | Renames or moves a file or directory to `dst` — and if `dst` is an existing directory, moves `src` *into* it keeping its basename (`mv notes.txt backup/`), like real `mv`. | A `dst` that exists and is an ordinary file is **replaced** (POSIX behaviour, since 2026-09-02); replacing a *directory*, or moving a directory onto an existing name, is still refused. On ext2 the replacement is near-atomic — one write re-points the destination's directory entry at the source's inode, so the name never resolves to nothing; FAT32/exFAT have no inode indirection, so there the old entry is removed before the new one is written and the name is briefly absent. `mv x x` is refused ("source and destination are the same") — and refused *again* inside `fsd`, since a remote 9P client does not go through `/bin/mv`. Moving a directory to a different parent correctly updates its own `..` entry, so `cd ..` inside it still resolves to the *new* parent afterward. No cycle detection beyond the trivial self-move guard. |
