...
(`mount -r`, remote `/proc`/`/net`/`/dev/cons`, and `cpu` remote
execution) shown here, [`manual.md`](manual.md)'s cluster section is the
task-oriented walkthrough with two-machine examples.
...
| `mount -r <host:port> <path>` | `mount -r 10.0.2.10:564 /mnt/a` | **Remote-mount another machine's 9P export** into this shell's namespace at `<path>` (cluster Phase 1c). A path under `<path>` then resolves to a `NETOP_RMOUNT` round trip through `netd` to `host:port`, so `ls /mnt/a` / `cat /mnt/a/file` read (and write) the *remote* machine's disk over TCP. | **Per-task**, inherited by spawned commands. Read+write (single-writer, no distributed lock). **Trusted-LAN, no authentication** — served to any peer. `host` is a dotted-quad IPv4; `port` defaults to 564. A disconnect fails the next op cleanly rather than hanging. See `manual.md`'s cluster section. |
