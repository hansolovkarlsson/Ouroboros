...
| `ps` | `ps` | One line per scheduler slot: `unused`, `runnable`, or `blocked (waiting for input)`. | The caller can't distinguish "running right now" from "runnable" — it is, by definition, the one running when it asks. Output is redirectable like any other command's. |
...
| `fg` | `fg <n>` | Hands the keyboard to task `n` — e.g. `exec /EFI/ORBS/SH.BIN` then `fg 2` gives a real nested shell session; its `exit` hands the keyboard back. | **Foreground a task that never reads input and the keyboard is unreachable until that task exits on its own** — there is no interrupt key (no Ctrl+C concept in this kernel yet). `fg` is for interactive programs; for anything else, `kill` it instead. Ownership reverts to this shell automatically when the foregrounded task exits or is killed. |
