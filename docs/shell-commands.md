...
| `ls` | `ls [-l] [-a] [path...]` | Lists its operands, **sorted by name** (case-insensitive), defaulting to the current directory. Default layout is multi-column (files plain, subdirectories suffixed `/`); **`-l`** is the long form — one entry per line with its type (`d`/`-`), size, and modified `YYYY-MM-DD HH:MM`. **`-a`** includes dotfiles (and `.`/`..`), hidden otherwise. Flags combine (`-la`). Takes **any number of operands** (so `ls *.txt` works): **file** operands are listed as a group, **directory** operands have their contents listed with a `name:` header when there's more than one operand. | The long form's size and time come from a `stat` of each entry (`NP_STAT`); the time shows only where the filesystem records one — **FAT32** does, so `ls -l` on the disk shows real dates, while exFAT/ext2/`/proc` currently show `-` (size and type are always real). Assumes an 80-column width. Truncates rather than erroring if a listing exceeds the 512-byte buffer. |
