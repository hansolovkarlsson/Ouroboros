...
**X25519 in `ed25519`, session-auth step 1.** RFC 7748's Montgomery ladder on
the crate's existing field arithmetic, refusing an all-zero shared secret,
checked against every RFC 7748 vector and three exchanges generated by OpenSSL.
On the guest one scalar multiplication is 236 µs and under 3 KB of stack.
Measuring where it would run is what found the park depth below.
