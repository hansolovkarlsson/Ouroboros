...
its own region (one 2MB→4KB split). A region fits one 2MB slot because
the loader refuses one that would not (`elf_region_size`'s bound, added
2026-09-13 after a 1.9MB `.bss` produced a two-slot region and the view
aliased it) and every region base is 2MB-aligned; `build_view` checks
containment itself and maps a region that fails EL0-inaccessible, loudly.
