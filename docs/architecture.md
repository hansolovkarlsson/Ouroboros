...
| 46 | `monotonic_us` | — | microseconds since boot (the ARM generic timer's free-running counter) | A high-resolution monotonic clock, unlike `get_ticks`'s 20 ms preemption tick — needed where a real elapsed duration matters (the network server's TCP round-trip-time estimation for the RTO is the first user; a fetch RTT of tens of ms is 1–2 of `get_ticks`'s ticks). Overflow-safe; only meaningful as a difference of two readings. Not gated — a monotonic clock is a harmless read for any task |
| 54 | `task_name` | task index, out ptr, out capacity | the name's true length (up to `capacity` bytes copied into the buffer), or `0` if the slot has no name | Read-only observability, the companion to `task_state` (18) that turns `ps`'s slot list into named processes. Returns the task's `argv[0]`: boot-loaded tasks (`shell`/`idle`/`fsd`/`cond`/`netd`/`accountd`) are named by the loader/kernel, spawned tasks carry the argv they were launched with, and unused/reaped-pending slots return `0`. Not gated — a name is harmless to read, like `task_state`. (Syscalls 47–53 — the argv/cwd/namespace family — and 57–60 — `yield` plus the environment-export family `env_stage`/`get_envc`/`get_env` — aren't listed as their own rows here; see `syscall-abi/src/lib.rs` for their contracts. `env_stage`/`get_envc`/`get_env` mirror the argv trio exactly (a `[count][len][NAME=VALUE]…` blob delivered per-task and fetched by the child), but the env is *latched* rather than passed a length, since `SPAWN`'s four args are full; `get_env`'s out-buffer is capped at `MAX_USER_LEN` (512) like every user pointer, so read one entry at a time) |
...
| 63 | `random` | out ptr, out capacity | bytes written into the buffer, or `RANDOM_UNAVAILABLE` (`u64::MAX`) when this machine has no entropy device | Hardware entropy from a **virtio-rng** device (`virtio_rng.rs`), the fix for clock-derived password salts. Deliberately *not* gated to one task the way `BLOCK_*`/`NET_*` are: entropy has no state to corrupt and every account tool needs it. **Absence is the ordinary case, not an error** — only a QEMU run started with `-device virtio-rng-device` has one (Parallels and the Pi expose no virtio-mmio at all), so callers degrade *loudly*: `accounts::salt_from` returns a `strong` flag and `passwd`/`useradd` print "no hardware RNG - using a weaker clock-derived salt". The device may return fewer bytes than asked, and the kernel does not pad — a partly-random value presented as a full one is the quiet weakness this exists to remove. (Syscalls 61–62 and 64 — `set_id`/`get_id`/`get_groups` — aren't listed as their own rows; see `syscall-abi/src/lib.rs`. One thing about `set_id` is worth stating here rather than leaving to the source, because it is the kind of ABI detail a caller gets wrong silently: **`arg2`/`arg3` are the supplementary group list, as a (pointer, count) pair.** Identity and membership therefore change in one call — a session can never keep the previous user's groups — but it also means passing `0, 0` does *not* mean "leave the groups alone", it means **clear them**. Setting a non-empty list is root-only even where the identity change itself would be allowed, since membership is a privilege grant; a non-root caller may pass only an empty list, which only ever removes privilege. `MAX_SUPP_GROUPS` is 8.) |
