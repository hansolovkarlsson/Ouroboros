...
| 63 | `random` | out ptr, out capacity | bytes written into the buffer, or `RANDOM_UNAVAILABLE` (`u64::MAX`) when this machine has no entropy device | Hardware entropy from a **virtio-rng** device (`virtio_rng.rs`), the fix for clock-derived password salts. Deliberately *not* gated to one task the way `BLOCK_*`/`NET_*` are: entropy has no state to corrupt and every account tool needs it. **Absence is the ordinary case, not an error** — only a QEMU run started with `-device virtio-rng-device` has one (Parallels and the Pi expose no virtio-mmio at all), so callers degrade *loudly*: `accounts::salt_from` returns a `strong` flag and `passwd`/`useradd` print "no hardware RNG - using a weaker clock-derived salt". The device may return fewer bytes than asked, and the kernel does not pad — a partly-random value presented as a full one is the quiet weakness this exists to remove. (Syscalls 61–62, `set_id`/`get_id`, aren't listed as their own rows; see `syscall-abi/src/lib.rs`.) |
| 64 | `set_groups` | gids ptr, count | `0`, or `SET_GROUPS_DENIED` (`u64::MAX`) for a non-root caller | **Root only**, like POSIX `setgroups`: group membership is a permission grant, so a task that could add its own groups could grant itself any group-readable file. That forces the ordering in `login`/`su` — set the groups while still root, *then* drop identity. Up to `MAX_SUPP_GROUPS` (8) gids, *additional* to the primary gid in the packed `set_id` word. Children inherit the list at `spawn`; the kernel clears it on task death, and the shell clears it on logout (the shell never dies, so the next session must not inherit the last one's memberships). |
| 65 | `get_groups` | task index, out ptr, capacity **in gids** | the task's true supplementary count (up to `capacity` gids copied), or `GET_ID_ERR` for an out-of-range task | Ungated like `get_id` — membership is not a secret, and `fsd` reads the *sender's* list on every permission check to decide the group triad (which now matches the caller's primary gid **or** any supplementary one). |
