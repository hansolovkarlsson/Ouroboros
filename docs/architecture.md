...
| 58 | `env_stage` | blob ptr, blob len | `0` or `SPAWN_ERROR` | Stages an **environment blob** for the next `spawn`, in the *same* encoding as an argv blob, each entry's bytes being a `NAME=VALUE` string. Unlike argv and cwd it is a **pending latch** rather than a length argument, because `spawn`'s four arguments are already full; staging again before the next spawn replaces it. Bounded in practice by **`MAX_USER_LEN` (512)**, not by `ENV_MAX` (2048): `valid_user_range` runs first and refuses any longer buffer, which makes the `ENV_STAGING_SIZE` check below it dead above 512. A blob over 512 bytes is therefore refused outright, and because the whole environment travels as one blob the child inherits **nothing** rather than a truncated set. The shell ignores this call's return value, so today that is silent. Treat 512 as the limit until the two bounds are reconciled |
