...
| [2026-09-23](2026-09-23.md) | 6 | the async arc gets its postmortem, the thirtieth: a blocking call is a lock nobody wrote down · X25519, and a stack gate that failed at the park (80 bytes), so a park never signs · the delegation race closed in libc, and a pass count sized to a rare control · HMAC-SHA-512 (step 2, 120 to 160 µs against a 536 µs sign), and an instrument that measured its neighbours · the boot identity (step 3), and the write-side rule the plan did not spell out · the keyed wire written down (step 4), and a checker taught to read it |
