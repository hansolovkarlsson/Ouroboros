...
| [2026-09-22](2026-09-22.md) | 10 | async remote mount step 2: the held fid session onto the parked engine, the two-phase park · the stack moved and only a boot showed how far (STACK_PAGES 12 to 14) · checks with measured controls, and a model handoff mid-session · the syscall table covers all 60, and a placeholder-cell rule question settled · step 3: the run parks, the drain goes, and a "latent" overflow turns out to break Phase 4b outright · two measurements that contradicted the plan and me · the environment that never arrived: a staging call bounded by the wrong constant, measured with a control | · the afternoon: a root cleanup and a PR its review closed · session-scoped authentication measured before it was planned (2,926 µs of every fid verb) · four reviews of a plan
| [2026-09-23](2026-09-23.md) | 9 | the async arc gets its postmortem, the thirtieth: a blocking call is a lock nobody wrote down · X25519, and a stack gate that failed at the park (80 bytes), so a park never signs · the delegation race closed in libc, and a pass count sized to a rare control · HMAC-SHA-512 (step 2, 120 to 160 µs against a 536 µs sign), and an instrument that measured its neighbours · the boot identity (step 3), and the write-side rule the plan did not spell out · the keyed wire written down (step 4), and a checker taught to read it · keyed sessions host to host (step 5), and a restore that did not restore · two reviews, and a trap I had written down · what the day was made of |
| [2026-09-26](2026-09-26.md) | 9 | step 6: the export keys a session, and a stack cost the disassembly found after two wrong guesses (an inlined `log`, then a second caller that un-inlined the dispatch) · step 6's review · step 7: the client, and a probe that measured its own console dump (the capture corrected it) · step 7's review, step 8, and v0.21.0 released · what the day was made of · then the arc's postmortem, the thirty-first: stateless was a guarantee, not an absence · per-user keys: the fork settled (password-derived keys as the foundation for an auth server) and root found claimable by any machine · four reviews of the plan, a rewrite of who holds the keys, and an offline-guessing exposure stated rather than cured · what the rest of the day was made of |
