...
//!   only for points outside the prime-order subgroup, which no honest signer
//!   produces. The cluster generates both keys in this crate, so the stricter
...
//! - **Small-order public keys are not rejected.** RFC 8032 does not require it,
//!   and this cluster's keys are generated by `keygen` from real entropy rather
//!   than accepted from strangers. If that ever stops being true — public keys
//!   arriving from somewhere less controlled — this is the check to add.
